...
const SHARED_CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;}
  ...
  /* Filter chips */
  .chips-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:8px;}
  @media(min-width:500px){.chips-grid{grid-template-columns:repeat(3,1fr);}}
  @media(min-width:768px){.chips-grid{display:flex;flex-wrap:wrap;}}
  .filter-chip{border-radius:12px;padding:10px 12px;display:flex;align-items:center;gap:8px;cursor:pointer;border:2px solid transparent;transition:border-color .15s,box-shadow .15s,transform .15s;background:#f8faff;}
  @media(min-width:768px){.filter-chip{flex:1;min-width:100px;}}
  .filter-chip:hover{transform:translateY(-1px);}
  .filter-chip.selected{border-color:currentColor;box-shadow:0 4px 14px rgba(0,86,179,.10);}
  /* Action buttons */
  .action-btn{border:none;border-radius:8px;padding:8px 0;font-size:12px;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;display:inline-flex;align-items:center;justify-content:center;gap:5px;transition:all .15s;white-space:nowrap;flex:1;}
  .action-btn:hover{opacity:.85;transform:translateY(-1px);}
  .action-btn:disabled{opacity:.5;cursor:not-allowed;transform:none;}
  .action-btn-sm{border:none;border-radius:8px;padding:5px 13px;font-size:12px;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;display:inline-flex;align-items:center;gap:5px;transition:all .15s;white-space:nowrap;}
...
function ConfirmModal({ title, body, confirmLabel, variant, onConfirm, onClose, busy }) {
  const bg = { danger: "#dc3545", warn: "#fd7e14", primary: "#0056b3" }[variant] ?? "#0056b3";
  return (
    <div className="modal-overlay" onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-box" onMouseDown={e => e.stopPropagation()}>
        <div className="modal-hdr">
          <h5 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "#0f172a" }}>{title}</h5>
          <button onClick={onClose} disabled={busy} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, color: "#94a3b8", padding: 4, lineHeight: 1 }}><i className="bi bi-x-lg"></i></button>
        </div>
        <div className="modal-body"><p style={{ margin: 0, fontSize: 13, color: "#64748b", lineHeight: 1.65 }}>{body}</p></div>
        <div className="modal-ftr">
          <button className="dash-btn-ghost" onClick={onClose} disabled={busy}>Cancel</button>
          <button className="dash-btn-primary" style={{ background: bg }} onClick={onConfirm} disabled={busy}>
            {busy ? <><span className="spinner-border spinner-border-sm me-2" style={{ width: "0.75rem", height: "0.75rem" }} />Working…</> : confirmLabel}
          </button>
        </div>
      </div>
    </div>
    ...
    <>
        ...
        <div className="d-flex">
          ...
          <main style={{ flex: 1, padding: "20px 16px", paddingBottom: 90, minWidth: 0 }}>
            ...
            <div className="d-lg-none mb-3" style={{ position: "relative" }}>
              ...
              <input className="dash-search" style={{ paddingLeft: 36 }} placeholder="Search exams…" value={search} onChange={e => setSearch(e.target.value)} />
            </div>
            ...
            {/* ── Status filter chips — 2-col grid on mobile, flex on desktop ── */}
            <div className="dash-card fade-up" style={{ padding: "12px", marginBottom: 14 }}>
              <div className="chips-grid">
                {STATUS_CHIPS.map(({ key, label, value, color, bg, icon }) => {
                  ...
                  return (
                    <div key={key} className={`filter-chip ${sel ? "selected" : ""}`}
              ...
              {(search || statusFilter !== "all") && (
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <button className="dash-btn-ghost" style={{ padding: "4px 10px", fontSize: 11 }} onClick={() => { setStatusFilter("all"); setSearch(""); }}>
            ...
            <div className="dash-card fade-up">
              ...
              {loading ? (
                ...
                <div style={{ padding: "40px 16px", textAlign: "center", color: "#94a3b8" }}>
                  ...
                  <p style={{ margin: 0, fontSize: 13 }}>{search || statusFilter !== "all" ? "No exams match your filters." : "No exams found."}</p>
                </div>
              ) : (
                <>
                  {/* ── Desktop table ── */}
                  <div className="hide-mobile" style={{ overflowX: "auto" }}>
                    <table className="dash-table">
                      <thead>
                        <tr>{["Exam", "Course", "Type", "Status", "Duration", "Submissions", "Flagged", "Actions"].map(h => <th key={h}>{h}</th>)}</tr>
                      </thead>
                      <tbody>
                        {pageData.map(exam => {
                          const isAct = exam.status === "active";
                          return (
                            <tr key={exam.id}>
                              <td>
                                <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: "#0f172a" }}>{exam.title}</p>
                                {exam.start_time && <p style={{ margin: "1px 0 0", fontSize: 11, color: "#94a3b8" }}>{new Date(exam.start_time).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}</p>}
                              </td>
                              <td>
                              ...
                              <td>{(exam.flagged_count ?? 0) > 0 ? <span style={{ fontSize: 14, fontWeight: 700, color: "#dc3545" }}>{exam.flagged_count}</span> : <span style={{ fontSize: 12, color: "#94a3b8" }}>0</span>}</td>
                              <td>
                                <div style={{ display: "flex", gap: 6 }}>
                                  <button className="action-btn-sm" onClick={() => setModal({ type: "toggle", exam })}
                                    style={{ background: isAct ? "#fff8f0" : "#f0fdf4", color: isAct ? "#fd7e14" : "#16a34a" }}>
                                    <i className={`bi ${isAct ? "bi-pause-circle" : "bi-play-circle"}`}></i>{isAct ? "Disable" : "Enable"}
                                  </button>
                                  <button className="action-btn-sm" onClick={() => setModal({ type: "reset", exam })}
                  ...
                  <div className="d-lg-none" style={{ padding: "12px", display: "flex", flexDirection: "column", gap: 10 }}>
                    {pageData.map(exam => {
                      const isAct = exam.status === "active";
                      return (
                        <div key={exam.id} className="exam-card">
                          <div style={{ padding: "14px 16px" }}>
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, marginBottom: 6 }}>
                              <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: "#0f172a", flex: 1, minWidth: 0, lineHeight: 1.3 }}>
                                {exam.title}
                              </p>
                            ...
                            {exam.course?.name && (
                              <p style={{ margin: "0 0 8px", fontSize: 12, color: "#64748b" }}>
                                <i className="bi bi-book me-1" style={{ color: "#94a3b8" }}></i>
                                {exam.course.name}
                                {exam.course.code && <span style={{ color: "#94a3b8" }}> · {exam.course.code}</span>}
                              </p>
                            )}

                            {/* Row 3: type + duration + flagged */}
                            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
                              ...
                              {exam.submissions_count > 0 && (
                                <span className="badge-pill" style={{ background: "#f0fdf4", color: "#16a34a" }}>
                                  <i className="bi bi-people"></i>{exam.submissions_count} submitted
                                </span>
                              )}
                            </div>
                            ...
                            <div style={{ display: "flex", gap: 8 }}>
                              <button className="action-btn"
                                ...
                                style={{ background: isAct ? "#fff8f0" : "#f0fdf4", color: isAct ? "#fd7e14" : "#16a34a" }}>
                                <i className={`bi ${isAct ? "bi-pause-circle" : "bi-play-circle"}`}></i>
      ...
      {modal?.type === "reset" && (
        <ConfirmModal
          ...
          confirmLabel="Yes, Reset Sessions" variant="danger"
