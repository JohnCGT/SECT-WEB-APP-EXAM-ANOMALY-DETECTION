...
export function AdminTopbar({ title, subtitle }) {
  return (
    <div style={{
      ...
      <div>
        <p style={{ margin: 0, fontSize: 16, fontWeight: 700, fontFamily: "'Syne',sans-serif", color: C.text }}>
          {title}
        </p>
    ...
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          ...
          {delta && (
            <p style={{ margin: "4px 0 0", fontSize: 12, color: C.muted }}>{delta}</p>
          )}
        </div>
        <span style={{
...
export default function AdminPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      // GET /api/admin/dashboard → { total_users, active_exams, flagged_sessions, high_cpi_risk, recent_results[] }
        ...
        <div style={{ flex: 1, padding: "28px 28px", overflowY: "auto" }}>
          ...
          <div style={{ display: "flex", gap: 14, marginBottom: 28, flexWrap: "wrap" }}>
            {[
              ...
              { to: "/admin/logs",      label: "System Logs",      color: C.muted   },
            ].map(({ to, label, color }) => (
